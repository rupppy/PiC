...
test_that("Forest_seg7 with realistic forest scene", {
  ...
  # Verifica output
  wood <- fread("test_forest_dim3_th1_Wood_eps2_mpts6.txt")
  floor_out <- fread("test_forest_Forest_floor_eps2_mpts6.txt")
  agb_no_wood <- fread("test_forest_dim3_th1_AGBnoWOOD_eps2_mpts6.txt")

  # Test sulla segmentazione
  expect_true(nrow(wood) > 1000)  # dovrebbe trovare abbastanza punti del tronco
  expect_true(nrow(floor_out) > 1000)  # dovrebbe trovare abbastanza punti del terreno
  expect_true(all(min(floor_out$z) < 1))  # il pavimento dovrebbe essere sotto 1m
  expect_true(max(wood$z) > 5)  # dovrebbe trovare tronchi alti almeno 5m
})
