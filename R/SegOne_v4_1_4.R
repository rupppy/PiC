...
#' @param R Numeric threshold for cluster shape parameter (standard deviation proportion 
...
SegOne <- function(a, 
  ...
  if (calculate_metrics) {
    ...
    if (!is.na(canopy_metrics$canopy_volume_m3)) {
      message("    Canopy volume: ", round(canopy_metrics$canopy_volume_m3, 2), " m^3")
      message("    Occupied volume: ", round(canopy_metrics$occupied_volume_m3, 2), " m^3")
      message("    Coverage area: ", round(canopy_metrics$coverage_area_m2, 2), " m^2")
...
#' 4. Filter clusters by minimum size (>= 20 voxels = 0.2 m^3)
#' 5. Filter clusters by minimum height (>= 1.0 m above base)
...
#'   20 voxels = 0.02 m^3 minimum crown volume
...
#' \itemize{
#'   \item Fine scanning (>2000 pts/m^2): 0.08-0.10 m
#'   \item Standard scanning (1000-2000 pts/m^2): 0.10-0.12 m
#'   \item Coarse scanning (<1000 pts/m^2): 0.12-0.15 m
#' }
#' 
...
#' 
#' \enumerate{
#'   \item \strong{Minimum height}: CBH >= 0.5 m (forestry standard)
#'   \item \strong{Maximum height}: CBH <= 90% tree height (biological constraint)
#'   \item \strong{Crown presence}: >= 30% of foliage points above CBH
#'   \item \strong{Cluster validity}: Primary cluster contains >= 20 voxels
...
#'   \item crown_volume: Approximate crown volume (m^3)
...
.calculate_crown_base_v4 <- function(tree_base_z, 
  ...
  if(nrow(agb_buffer) < 50) {
    message("    X Insufficient foliage points in buffer: ", nrow(agb_buffer))
    if(diagnostics) {
  ...
  if(nrow(voxel_centroids_filtered) < 20) {
    message("    X Too few voxels after density filtering: ", nrow(voxel_centroids_filtered))
  ...
  if(n_clusters == 0) {
    message("    X No clusters detected")
  ...
  for(i in 1:min(5, nrow(cluster_stats))) {
    message("      Cluster ", cluster_stats$cluster[i], ": ",
            ...
            "vol = ", round(cluster_stats$volume_m3[i], 2), " m^3")
  }
  ...
  message("    Clusters >= ", min_cluster_voxels, " voxels: ", nrow(valid_size))
  
  if(nrow(valid_size) == 0) {
    message("    X No clusters meet size threshold")
  ...
  message("    Clusters with min_z >= ", min_crown_height, " m: ", nrow(valid_clusters))
  
  if(nrow(valid_clusters) == 0) {
    message("    X No clusters above height threshold")
  ...
  message("    Crown volume: ", round(primary_crown$volume_m3, 2), " m^3")
  ...
  if(cbh_candidate < min_cbh) {
    message("    X Crown base too low: ", round(cbh_candidate, 2), 
            " m (minimum: ", min_cbh, " m)")
    if(diagnostics) {
  ...
  if(cbh_candidate > max_cbh) {
    message("    X Crown base too high: ", round(cbh_candidate, 2), 
  ...
  if(foliage_ratio < 0.3) {
    message("X Insufficient foliage above crown base: ", 
...
.diagnose_crown_clustering_v4 <- function(diagnostic_data, tree_base, 
  ...
  summary_text <- paste0(
    ...
    "  Crown volume: ", diagnostic_data$crown_volume_m3, " m^3\n",
...
.calculate_canopy_volume <- function(foliage_points, tree_base_z, 
  ...
  tryCatch({
    ...
    if(nrow(crown_points) < 10) {
      message("    X Insufficient crown points: ", nrow(crown_points))
      return(result)
    }
    ...
    message("Canopy analysis completed")
    
    return(result)
    
  }, error = function(e) {
    message("    X Canopy analysis error: ", e$message)
    return(result)
  })
}
